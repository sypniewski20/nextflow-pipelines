...
process JOINT_SV2_REGENOTYPE {
	label 'sv2'	
	label 'mem_16GB'
	label 'core_16'
	input:
		path(bam)
		tuple path(delly), path(delly_tbi)
		tuple path(manta), path(manta_tbi)
		tuple path(cnvpytor), path(cnvpytor_tbi)
		tuple path(erds), path(erds_tbi)
		tuple path(snv), path(snv_tbi)
		path(ped_file)
		path(fasta)
	output:
        path("sv2_genotypes/${sample}_sv2.vcf")
	script:
		"""
...
process JOINT_SV2_FILTER_VCF {
	...
	input:
		path(sv2)
	output:
		tuple path("${sample}_sv2_sorted.vcf.gz"), path("${sample}_sv2_sorted.vcf.gz.tbi"), emit: vcf
		path("${sample}_sv2.tsv"), emit: tsv
	script:
		"""
		...
		bcftools sort ${sv2}.gz -Ov | \
		bcftools view -f PASS --threads ${task.cpus} -Oz -o multisample_sv2_sorted.vcf.gz

		tabix -p vcf multisample_sv2_sorted.vcf.gz

		echo -e "SAMPLE\tCHROM\tPOS\tID\tSVTYPE\tSVLEN\tCYTOBAND\tDESCRIPTION\tGT\tCN" > multisample_sv2.tsv

		bcftools query -f '[%SAMPLE\t%CHROM\t%POS\t%ID\t%SVTYPE\t%SVLEN\t%CYTOBAND\t%DESCRIPTION\t%GT\t%CN\n]' multisample_sv2_sorted.vcf.gz >> multisample_sv2.tsv
